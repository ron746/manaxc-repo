...
for division in divisions:
    ...
    for match in result_pattern.finditer(division_text):
        ...
        # Format time (preserve all decimals, ensure mm:ss.xx)
        try:
            ...
            time = time_raw
        # Graduation year using athletic calendar
        try:
            grade_int = int(grade)
            ...
            graduation_year = "UNK"
        # Split name, remove nicknames in parentheses
        ...
        if len(name_parts) == 1:
            ...
            athlete_first_name = name_parts[0]
            athlete_last_name = " ".join(name_parts[1:])
        results.append({
            "place": place,
            "grade": grade,
            "time": time,
