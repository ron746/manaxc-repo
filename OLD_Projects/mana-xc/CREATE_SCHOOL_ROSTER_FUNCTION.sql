...
BEGIN
  ...
  WITH athlete_season_stats AS (
    SELECT
      ...
      a.full_name,
      CASE
        WHEN a.gender::text = 'M' THEN 'M'
        WHEN a.gender::text = 'F' THEN 'F'
        WHEN a.gender = false THEN 'M'
        WHEN a.gender = true THEN 'F'
        ELSE 'M'
      END as gender,
...
-- Replace with actual school_id from your database
SELECT * FROM get_school_roster(
  (SELECT id FROM schools WHERE name LIKE '%Westmont%' LIMIT 1),
  2025
);
