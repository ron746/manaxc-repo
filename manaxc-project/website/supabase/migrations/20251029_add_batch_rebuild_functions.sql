-- Add RPC functions for batch rebuild operations
-- These can be called from API routes for admin batch processing
...
BEGIN
  -- Use a CTE to calculate best times, then upsert
  WITH athlete_stats AS (
    SELECT
      r.athlete_id,
      m.season_year,
      MIN(r.time_cs) FILTER (WHERE r.time_cs IS NOT NULL) as season_best_time_cs,
      MIN(r.normalized_time_cs) FILTER (WHERE r.normalized_time_cs IS NOT NULL) as season_best_normalized_time_cs,
      (SELECT id FROM results WHERE athlete_id = r.athlete_id AND time_cs = MIN(r.time_cs) FILTER (WHERE r.time_cs IS NOT NULL) ORDER BY meet_date DESC LIMIT 1) as season_best_result_id,
      MIN(r.time_cs) OVER (PARTITION BY r.athlete_id) as alltime_best_time_cs,
      MIN(r.normalized_time_cs) OVER (PARTITION BY r.athlete_id) as alltime_best_normalized_time_cs,
      (SELECT id FROM results r2 JOIN meets m2 ON r2.meet_id = m2.id WHERE r2.athlete_id = r.athlete_id AND r2.time_cs = MIN(r.time_cs) OVER (PARTITION BY r.athlete_id) ORDER BY m2.meet_date DESC LIMIT 1) as alltime_best_result_id
    FROM results r
