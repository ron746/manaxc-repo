-- Batch rebuild all derived tables after bulk import - CORRECTED VERSION v3
...
-- =============================================================================
-- 2. REBUILD ATHLETE_BEST_TIMES (simplified - separate season and alltime)
-- =============================================================================

-- Insert/update athlete best times
WITH season_stats AS (
    -- Get season best times
    SELECT
        r.athlete_id,
        m.season_year,
...
alltime_stats AS (
    -- Get all-time best times
    SELECT
        athlete_id,
        MIN(time_cs) as alltime_best
    FROM results
    WHERE time_cs IS NOT NULL
    GROUP BY athlete_id
),
season_result_ids AS (
    -- Get result IDs for season bests
...
JOIN season_result_ids sr ON ss.athlete_id = sr.athlete_id AND ss.season_year = sr.season_year
JOIN alltime_stats ats ON ss.athlete_id = ats.athlete_id
JOIN alltime_result_ids ar ON ss.athlete_id = ar.athlete_id
