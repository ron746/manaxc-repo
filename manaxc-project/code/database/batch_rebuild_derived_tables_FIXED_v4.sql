-- Batch rebuild all derived tables after bulk import - CORRECTED VERSION v4
...
-- =============================================================================
-- 2. REBUILD ATHLETE_BEST_TIMES (with all required columns)
-- =============================================================================

-- Insert/update athlete best times
WITH season_stats AS (
    -- Get season best times with all metadata
    SELECT
        r.athlete_id,
        m.season_year,
        MIN(r.time_cs) as season_best_time,
        MIN(r.normalized_time_cs) as season_best_normalized
    FROM results r
...
alltime_stats AS (
    -- Get all-time best times with all metadata
    SELECT
        athlete_id,
        MIN(time_cs) as alltime_best_time,
        MIN(normalized_time_cs) as alltime_best_normalized
    FROM results
